...
public class GameStateRunning implements BaseState<Game> {
  ...
  Vector2D mouseStart = new Vector2D();
  ...
  public void Update(Game owner, Game game) {
    ...
    while (System.currentTimeMillis() > next_game_tick && loops < MAX_FRAMESKIP) {
      ...
      Vector2D mouseStart = new Vector2D(game.container.getInput().getAbsoluteMouseX() + game.camera.getCamX(), game.container.getInput().getAbsoluteMouseY() + game.camera.getCamY());

      System.out.println(mouseStart.x + "  " + mouseStart.y);
      
      
      if (game.container.getInput().isKeyPressed(Input.KEY_H)) {
        IntStream.range(0, 10).forEach(e -> game.world.eManager.addZombie(new Zombie(new Vector2D(new Random().nextInt(100) + mouseStart.x, new Random().nextInt(100) + mouseStart.y))));
      }
      ...
      if (game.container.getInput().isMousePressed(1)) {
        Vector2D additonalInfo = new Vector2D(game.container.getInput().getAbsoluteMouseX() + game.camera.getCamX(), game.container.getInput().getAbsoluteMouseY() + game.camera.getCamY());
      ...
      EventDispatcher.getEvents().forEach(e -> {
        switch (e.getEvent()) {
          ...
          case FireAt:
            game.world.eManager.addBullet(new Bullet((Vector2D) e.getAdditionalInfos().get(0), (Vector2D) e.getAdditionalInfos().get(1), (int) e.getAdditionalInfos().get(2), (int) e.getAdditionalInfos().get(3)));
            EventDispatcher.removePersistentEvent(e);
       ...
       * 
       * @Override public void setInput(Input input) { // TODO Auto-generated method stub
       * 
       * }
       * 
       * @Override public boolean isAcceptingInput() { // TODO Auto-generated method stub return
       * true; }
       * 
       * @Override public void inputStarted() { // TODO Auto-generated method stub
       * 
       * }
       * 
       * @Override public void inputEnded() { // TODO Auto-generated method stub
       * 
       * }
       * 
       * @Override public void mouseWheelMoved(int change) { // TODO Auto-generated method stub
       * 
       * }
       * 
       * @Override public void mouseReleased(int button, int x, int y) { if (button == 0) { Vector2D
       * additonalInfo = game.camera.getPositionInWorld(mouseStart); EventDispatcher.createEvent(0,
       * EventType.LeftUp, additonalInfo); }
       * 
       * }
       * 
       * @Override public void mousePressed(int button, int x, int y) { if (button == 0) { Vector2D
       * additonalInfo = game.camera.getPositionInWorld(mouseStart); EventDispatcher.createEvent(0,
       * EventType.LeftDown, additonalInfo); } }
       * 
       * @Override public void mouseMoved(int oldx, int oldy, int newx, int newy) {
       * System.out.println("WorldPos: X: " + game.camera.getWorldPosX(newx) + "  Y: " +
       * game.camera.getWorldPosY(newy)); System.out.println("ScreenPos: X: " + newx + "  Y: " +
       * newy);
       * 
       * 
       * }
       * 
       * @Override public void mouseDragged(int oldx, int oldy, int newx, int newy) { if
       * (t2.getSeconds() >= 0.1) {
       * 
       * Vector2D additonalInfo = new Vector2D(game.camera.getWorldPosX(newx),
       * game.camera.getWorldPosY(newy)); EventDispatcher.createEvent(0, EventType.LeftDrag,
       * additonalInfo); t2.restart(); } }
       * 
       * @Override public void mouseClicked(int button, int x, int y, int clickCount) { // TODO
